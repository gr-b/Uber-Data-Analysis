...
#[0.start_date, 1.end_date, 2.start_long, 3.start_lat, 4.end_long, 5.end_lat, 6.taxi_dist, 7.tip_amount, 8.taxi_start(sec), 9.taxi_duriation(sec), 10.taxi_google_start_coords, 11.taxi_google_end_coords, 12.google_dist, 13.google_dur]

#key = '' #Put your google API key here. You are limited to ~2000 per day so be careful what you put in

max_read = 1000 #The max driving events to read from a spreadsheet (So we don't overload my google API quota)
    ...
    with open(fileName) as csvfile: #Open the CSV File
        ...
        for row in reader: #Get data from the csv file
            ...
            rowData = [row['lpep_pickup_datetime'], row['Lpep_dropoff_datetime'],
                       ...
                       row['Trip_distance'], row['Tip_amount']]
...
def getDataMany(data):
    for i, event in enumerate(data):
        curEvent = getData(event[10], event[11], str(int(float(event[8]))))
...
def getJSONData(json_data):
    ...
    duration = parsed_json['routes'][0]['legs'][0]['duration']['value']
    retArray = [distance, duration]
    return retArray
...
def calcStats(data):
    ...
    for i, event in enumerate(data):
        insert = []
        print "Dist, Record:" + str(data[i][6]) + ", Google:" + str(data[i][12]) + "Tip:" + str(data[i][7])
        print "Time, Record:" + str(data[i][9]) + ", Google:" + str(data[i][13]) + "Tip:" + str(data[i][7])
        if (data[i][11] != 0 and data[i][13] != 0) :
            distRatio = float(data[i][6])/data[i][12]
            timeRatio = float(data[i][9])/data[i][13]
            if timeRatio < 10.0 and distRatio < 10.0 :
...
def main():
    ...
    Input = readCSV('green_tripdata_2013-08.csv')
    print "Done Reading"
    ...
    print "Done collecting Google Data"
    #print GoogleData
